...
int singleNumber(const vector<int> &A) {
    int ans=0;
    size_t N = A.size();
    for(int i=0;i<32;i++) // ioterating all the bits
    {
        int sum=0;
        for(int j=0;j<N;j++)
        {
...
int main(int argc, const char * argv[]) {
    // insert code here...
    std::cout << "Hello, find sinlge number not repeatingthrice!\n";
    vector<int> A/*{2,2,2,5,5,5,3}*/
    ...
    cout<<"Number repeating thrice : "<<singleNumber(A)<<endl;
    return 0;
}
