...
int solveByPrims(int A, int S, vector<vector<int> > &B) {
    int ans=0;
    //Step 1 - create adjancency list
    //vector<pair{node, weight})
    auto E = B.size(); // number of edges
    vector<pair<int, int>> G[A+1];
    for(int i=0;i<E;i++)
    {
        G[B[i][0]].push_back({B[i][1], B[i][2]}); // B[i][0] : U -----> B[i][1] : V with weight B[i][2] : W
        G[B[i][1]].push_back({B[i][0], B[i][2]}); //Undirected graph
    }
    
    //Step 2 create visited list
    auto vis = vector<int>(A+1, 0); //Zero being not visited
    vis[S]=1; // AS we starting form S, otherwise i can get min out of all edges first to start with by iterating over the edges
    
    //creating MinHeap so we can fetch shortest path like greedy algo
    auto minHeap = priority_queue<pair<int, pair<int, int>>, vector<pair<int, pair<int, int>>>, greater<pair<int, pair<int, int>>>>();
    for(auto [ver, wgh] : G[S])  // pushing vertices adjacent to source node given
        minHeap.push({wgh, {S, ver}});
    
    // creating MST using BFS
    
    while(!minHeap.empty())
    {
        auto [W, ind] = minHeap.top(); minHeap.pop();
        if(vis[ind.second]==0)
        {
            ans+=W;
            vis[ind.second]=1; // marling as visited
        }
        
        for(auto [ver, wgh] : G[ind.second]) // getting adjacent vertices from V
        {
            if(vis[ver]==0){
                minHeap.push({wgh,{ind.second, ver}});
            }
        }
    }
