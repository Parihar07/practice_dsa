...
//  TreeTraversals
//
//  Created by CodeBreaker on 27/04/25.
//
...
void inOrderTreversal(Node *root) // Right -> Data -> Left
{
    if(root == NULL) return;
...
void preOrderTreversal(Node *root) // Data -> left -> right
{
    if(root == NULL) return;
    cout<<root->data<<" ";
...
void postOrderTreversal(Node *root) // Data -> left -> right
{
    if(root == NULL) return;
...
int main(int argc, const char * argv[]) {
    // insert code here...
    std::cout << "Hello, Basic tree traversals!\n";
    Tree tree;
    tree.createRandomTree();
    Node *rt = tree.getRoot();
    inOrderTreversal(rt); cout<<endl;
//    preOrderTreversal(rt); cout<<endl;
//    postOrderTreversal(rt); cout<<endl;
    morrisInorderTraversal(rt); cout<<endl;
    return 0;
}
